...
const App = props => {
  ...
  const isSqlPanelOpen = useSelector(
    state => duckDbPluginEnabled && state?.demo?.keplerGl?.map?.uiState?.mapControls?.sqlPanel?.active || false
  );

  const isAiAssistantPanelOpen = useSelector(
    state => state?.demo?.keplerGl?.map?.uiState?.mapControls?.aiAssistant?.active || false
  );

  const prevQueryRef = useRef<number>(null);

  useEffect(() => {
    console.log('AppEmbedded useEffect - props:', {id, provider, query});
...
// Create a wrapper component that provides the expected props structure
const AppEmbeddedWrapper = (props) => {
  // Debug: Log the Redux state to see what's available
  console.log('AppEmbedded Redux state:', props);
  console.log('AppEmbedded keplerGl state:', props?.demo?.keplerGl);
  
  // Check if kepler.gl state is properly initialized
  const hasKeplerState = props?.demo?.keplerGl?.map;
  console.log('Has kepler state:', hasKeplerState);
  
  // Instead of blocking, let's allow the component to render and initialize the state
  // The KeplerGl component will handle the state initialization when it mounts
  if (!hasKeplerState) {
    console.log('Kepler.gl state not yet initialized, but allowing component to render and initialize...');
    // Don't return early - let the component render and initialize
  }
  
  // Provide default props that the original App expects from router
  const defaultProps = {
    params: {},
    location: { query: {} },
    ...props
...
export default connect(mapStateToProps, dispatchToProps)(AppEmbeddedWrapper);
